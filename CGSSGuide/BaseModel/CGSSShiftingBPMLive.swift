...
class CGSSShiftingBPMLive: CGSSLive {
    ...
    static func checkIsShifting(live:CGSSLive) -> Bool {
        let dao = CGSSDAO.sharedDAO
        let maxDiff = (live.masterPlus == 0) ? 4 : 5
        if let beatmap = dao.findBeatmapById(live.id!, diffId: maxDiff) {
            var noteIntervals = [Float]()
            let firstNoteIndex = beatmap.notes.indexOf(beatmap.firstNote!) ?? 0
            let lastNoteIndex = beatmap.notes.indexOf(beatmap.lastNote!) ?? 0
