...
class EventPtView: UIView {
    ...
    func setup(rankingList: EventPtRankingList, onGoing: Bool) {
        dateLabel.text = rankingList.lastDate?.toString(format: "yyyy-MM-dd HH:mm")
        ...
        if let last = rankingList.last {
            ...
            if onGoing {
                ...
                gridStrings.append(["2000", String(last.reward1), "\(rankingList.speed.reward1)"])
                gridStrings.append(["10000", String(last.reward2), "\(rankingList.speed.reward2)"])
                gridStrings.append(["20000", String(last.reward3), "\(rankingList.speed.reward3)"])
                gridStrings.append(["60000", String(last.reward4), "\(rankingList.speed.reward4)"])
                gridStrings.append(["120000", String(last.reward5), "\(rankingList.speed.reward5)"])
            } else {
                gridStrings.append(["", NSLocalizedString("最终Pt", comment: ""), "-"])
                gridStrings.append(["1", String(last.rank1), "-"])
                gridStrings.append(["2000", String(last.reward1), "-"])
                gridStrings.append(["10000", String(last.reward2), "-"])
                gridStrings.append(["20000", String(last.reward3), "-"])
                gridStrings.append(["60000", String(last.reward4), "-"])
                gridStrings.append(["120000", String(last.reward5), "-"])
            }
