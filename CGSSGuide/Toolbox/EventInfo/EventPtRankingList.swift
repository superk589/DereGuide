...
extension EventPtRankingList {
    ...
    var last: EventPtRanking? {
        return list.last
    }
    
    var speed: EventPtRankingSpeed {
        var subList = [EventPtRanking]()
        var count = 0
...
class EventPtRankingList {
    ...
    init(fromJson json: JSON!){
        ...
        for sub in json.arrayValue {
            ...
            list.append(EventPtRanking.init(fromJson: sub))
        }
    }
    ...
    {
        list = aDecoder.decodeObject(forKey: "list") as? [EventPtRanking] ?? [EventPtRanking]()
    }
