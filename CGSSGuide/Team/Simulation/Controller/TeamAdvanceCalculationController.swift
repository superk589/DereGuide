//
//  TeamAdvanceCalculationController.swift
//  CGSSGuide
//
...
class TeamAdvanceCalculationController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(TeamAdvanceCalculationCell.self, forCellReuseIdentifier: TeamAdvanceCalculationCell.description())
        tableView.register(TeamAdvanceDescriptionCell.self, forCellReuseIdentifier: TeamAdvanceDescriptionCell.description())
    }
}

extension TeamAdvanceCalculationController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            let cell = tableView.dequeueReusableCell(withIdentifier: TeamAdvanceDescriptionCell.description(), for: indexPath) as! TeamAdvanceDescriptionCell
            let row = descriptionSection[indexPath.row]
            cell.setupWith(leftString: row.name, rightString: String(row.value))
            return cell
        } else {
            let cell = tableView.dequeueReusableCell(withIdentifier: TeamAdvanceCalculationCell.description(), for: indexPath) as! TeamAdvanceCalculationCell
...
extension TeamAdvanceCalculationController: TeamAdvanceCalculationCellDelegate {
    ...
    func teamAdvanceCalculationCell(_ teamAdvanceCalculationCell: TeamAdvanceCalculationCell, didStartCalculationWith varibles: [String]) {
        guard let indexPath = tableView.indexPath(for: teamAdvanceCalculationCell) else {
            return
        }
        ...
        case 0:
            if validateDoublePercentInput(variable: varibles[0]) {
                ...
                teamAdvanceCalculationCell.updateResult(calculationSection[indexPath.row])
            }
        case 1:
            if validateDoublePercentInput(variable: varibles[0]) {
                let x = Double(varibles[0])!
        ...
        case 3:
            if validateDoubleOrNilInput(variable: varibles[0]) && validateDoubleOrNilInput(variable: varibles[1]) {
                ...
                let b = Double(varibles[1]) ?? Double.greatestFiniteMagnitude
                calculationSection[indexPath.row].variables = varibles
                teamAdvanceCalculationCell.startCalculationAnimating()
                DispatchQueue.global(qos: .userInteractive).async {
                    ...
                    DispatchQueue.main.async {
                        teamAdvanceCalculationCell.stopCalculationAnimating()
                        teamAdvanceCalculationCell.updateResult(self.calculationSection[indexPath.row])
                    }
                }
            }
        default:
